...
export default {
    ...
    methods: {
        buyElement(element_id, element_price, element_name){
            if (element_price > this.Coins) {alert("Not enough coins!"); return;}
            if (confirm("Press confirm to buy " + element_name) == false) {return;}
            for(let i = 0; i < this.elements_array.length; i++){
                if (this.elements_array[i].id != element_id) {continue;}
