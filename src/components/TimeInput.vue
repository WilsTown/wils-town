    ...
    watch: {
        time_val: {
            handler(val) {
                ...
                this.time_raw =
                    Number(this.time_val.slice(0, this.time_val.indexOf(":"))) *
                        60 +
                    Number(this.time_val.slice(this.time_val.indexOf(":") + 1));
                this.$emit("update", this.time_raw);
            },
        },
    },
    mounted() {
        this.time_val = String(Math.floor(this.SavedPeriod / 60)).concat(
            ":",
            this.SavedPeriod % 60 < 10
                ? "0" + (this.SavedPeriod % 60)
                : this.SavedPeriod % 60
        );
    },
};
